...
class ViterbiDecoder:
    ...
    def __init__(self, K, glist):
        ...
        # print "glist", glist
        # print "predecessor_states", self.predecessor_states
        # print "\n"
        ...
        # print "expected_parity", self.expected_parity
    ...
    def branch_metric(self,expected,received):
        #your code here
        digitized_voltages = [1 if voltage >= 0.5 else 0 for voltage in received]
        return PS3_tests.hamming(expected, digitized_voltages)
    ...
    def viterbi_step(self,n,received_voltages):
        ...
        for s in xrange(self.nstates):
            #pred means the predecessor
            pred_1 = self.predecessor_states[s][0]
            pred_2 = self.predecessor_states[s][1]
            
            path_1 = self.PM[pred_1, n-1] + self.branch_metric(self.expected_parity[pred_1][s], received_voltages)
            path_2 = self.PM[pred_2, n-1] + self.branch_metric(self.expected_parity[pred_2][s], received_voltages)
            
            if path_1 <= path_2:
                self.PM[s, n] = path_1
                self.Predecessor[s, n] = pred_1
            else:
                self.PM[s, n] = path_2
                self.Predecessor[s, n] = pred_2
    ...
    def most_likely_state(self,n):
        ...
        for state in xrange(self.nstates):
            if self.PM[smallest, n] > self.PM[state, n]:
                smallest = state
        return smallest
    ...
    def traceback(self,s,n):
        ...
        for i in reversed(xrange(1, n+1)):
            ...
            # print self.Predecessor[s, i]
            pred = self.Predecessor[s, i]
            for arc in xrange(self.nstates):
                if self.expected_parity[pred][arc] != None:
                    if arc == s:
